...
UserUtil.getUserName = function(user){
  return user && user.username? user.username || UserUtil.getProperty(user, 'services.twitter.screenName') : '';
}
...
UserUtil.getProfileUrl = function(user) {
  return Meteor.absoluteUrl()+'users/' + URLUtil.slugify(UserUtil.getUserName(user));
}
...
UserUtil.getTwitterName = function(user){
  ...
  if(HashUtil.checkHash(user) && URLUtil.checkNested(user, 'profile', 'twitter')){
...
UserUtil.getGitHubName = function(user){
  ...
  if(HashUtil.checkHash(user) && URLUtil.checkNested(user, 'profile', 'github')){
...
UserUtil.getGoogleName = function(user){
  ...
  if(HashUtil.checkHash(user) && URLUtil.checkNested(user, 'profile', 'google')){
...
UserUtil.getFacebookName = function(user){
  ...
  if(HashUtil.checkHash(user) && URLUtil.checkNested(user, 'profile', 'facebook')){
...
UserUtil.getEmail = function(user){
  if(HashUtil.checkHash(user, 'profile') && user.profile.email){
    return user.profile.email;
  } else if(HashUtil.checkHash(user) && HashUtil.checkArray(user.emails)){ 
    return user.emails[0].address; 
  } else {
    return '';
  }
}
UserUtil.findByEmail = function (userEmail){
	return Meteor.users.findOne({'user.profile.email':userEmail});
}
UserUtil.getAvatarUrl = function(user){
  if(UserUtil.getSignupMethod(user)=='twitter'){
    ...
    return  URLUtil.SITE_URL + 'img/ghost_avatar.png';
  }
}
...
UserUtil.getLocale = function(user) {
  return user != undefined && user.profile != undefined && user.profile.locale ? user.profile.locale : 'en';
  //return user == undefined || user.profile != undefined && user.profile.locale == undefined ? 'en' : user.profile.locale;
}
...
UserUtil.getUserIdOrSlug = function(user) {
  return user.profile && user.profile.slug ? user.profile.slug : user._id;
}

UserUtil.getState = function(user) {
  return user.profile && user.profile.state ? user.profile.state : '';
}

UserUtil.getCountry = function(user) {
  return user.profile && user.profile.country ? user.profile.country : '';
}

UserUtil.getSex = function(user) {
  return user.profile && user.profile.sex ? user.profile.sex : '';
}

UserUtil.canCreateTags = function(user) {
  return user.profile && user.profile.canCreateTags ? user.profile.canCreateTags : false;
}
